...
export function buildUserPrompt(inputs: PromptInputs): string {
  ...
  sections.push(
    [
      ...
      '- Set "linkedinHook" to the first line, exactly as it appears in the post.',
    ].join('\n'),
  );
  ...
  sections.push(
    [
      ...
      'a legible headline and no dense text. Never a screenshot, never a stock photo.',
      'The visual must complement the caption, not restate it.',
