...
_TMP = Path(tempfile.mkdtemp(prefix="microns-dental-tests-"))
...
os.environ.update(
    {
        ...
        # Unset (rather than the "primary" default) so calendar-touching code
        # paths take their "not configured" branch instead of attempting a
        # real Google API call with no OAuth token present.
...
def patient(db) -> Patient:
    record = Patient.create(phone="+15551234567", name="Jane Doe", email="jane@example.com", sms_consent=True)
