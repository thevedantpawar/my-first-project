...
class Patient(Base):
    ...
    Name, phone, email, insurance member id and treatment history are PHI and
    are stored encrypted. Because Fernet ciphertext is non-deterministic,
    lookups go through the ``*_fingerprint`` columns — a keyed HMAC of the
    ...
    encrypted_member_id = Column("encrypted_member_id", EncryptedString(512), nullable=True)
    ...
    #: Plan name only ("Delta Dental PPO") — not PHI on its own.
    ...
    external_id = Column(String(120), nullable=True, index=True)  # PMS id
    google_contact_resource_name = Column(String(200), nullable=True)
