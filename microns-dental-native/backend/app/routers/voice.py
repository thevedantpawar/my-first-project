...
VAPI sends with every server request when you configure a server secret on
the assistant.
"""
...
async def inbound(
    ...
    return VoiceInboundResponse(
        call_record_id=result["call_record_id"], assistant_overrides=result["assistant_overrides"],
        known_patient=result["known_patient"], greeting=result["greeting"],
    )
...
async def action(
    ...
    result = VoiceService(db, audit).handle_action(action=action_name, parameters=parameters, call_id=call_id)
...
def _parse_end_payload(payload: dict[str, Any]) -> VoiceEndRequest:
    ...
    duration = message.get("durationSeconds") or payload.get("duration_seconds") or payload.get("durationSeconds")
    try:
    ...
    return VoiceEndRequest(
        call_id=call.get("id") or payload.get("call_id"), transcript=transcript, duration_seconds=duration,
        outcome=payload.get("outcome"), ended_reason=message.get("endedReason") or payload.get("ended_reason"),
        summary=summary,
    )
