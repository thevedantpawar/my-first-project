"""Operator CLI.
    ...
    docker compose run --rm backend python -m app.cli gen-token
...
def cmd_google_auth(_args) -> int:
    """Run the interactive OAuth flow and write GOOGLE_TOKEN_PATH.

    Requires a browser on the machine running this command (or an SSH
    port-forward — see the README's "headless" note). Reads
    GOOGLE_CREDENTIALS_PATH (the OAuth client secret downloaded from Google
    Cloud Console) and writes GOOGLE_TOKEN_PATH.
    """
    from app.services.google_auth_service import GoogleAuthService

    service = GoogleAuthService()
    service.run_interactive_flow()
    print(f"Wrote refreshable Google OAuth token to {settings.google_token_path}")
    return 0
...
def cmd_seed_demo(args) -> int:
    ...
    Useful for exercising the dashboard before a practice's real data is
    loaded. Refuses to run in production.
    """
    ...
    try:
        now = utcnow()
        specimens = [
            ("Ava Thompson", "+15551230001", "cleaning", now + timedelta(hours=24), AppointmentStatus.CONFIRMED),
            ("Noah Patel", "+15551230002", "filling", now + timedelta(hours=2), AppointmentStatus.CONFIRMED),
            ("Mia Rodriguez", "+15551230003", "cleaning", now - timedelta(days=1), AppointmentStatus.NO_SHOW),
            ("Liam Chen", "+15551230004", "cleaning", now - timedelta(days=185), AppointmentStatus.COMPLETED),
            ("Zoe Bennett", "+15551230005", "crown", now - timedelta(days=10), AppointmentStatus.COMPLETED),
        ]
        created = 0
        for name, phone, service, when, status in specimens:
            patient = Patient.create(phone=phone, name=name, sms_consent=True)
        ...
        for index, (interest, pain, timeline) in enumerate(
            [("implants", "none", "today"), ("whitening", "none", "browsing"), ("emergency", "severe", "today")]
        ):
            ...
            lead.last_visit = "2_plus_years"
            lead.insurance_type = "ppo"
            lead.pain_level = pain
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="app.cli", description="Microns Dental Native operator CLI")
    ...
    subparsers.add_parser(
        "google-auth", help="run the interactive Google OAuth flow and write token.json"
