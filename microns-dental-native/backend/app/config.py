...
class Settings(BaseSettings):
    ...
    log_level: str = "INFO"
    app_name: str = "Microns Dental Native"
    cors_origins: Annotated[List[str], NoDecode] = ["http://localhost:3000", "http://localhost:8000"]
    allowed_hosts: Annotated[List[str], NoDecode] = ["*"]

    # --- Database ----------------------------------------------------------
    database_url: str = "postgresql://microns:password@postgres:5432/microns_dental_db"
    ...
    #: Shared secret for front-desk/dentist-facing endpoints (dashboard, at-risk patients).
    ...
    #: Whether SMS copy may name the treatment ("your root canal follow-up").
    ...
    on_call_transfer_number: Optional[str] = None

    # --- Calendly --------------------------------------------------------------
    calendly_api_key: Optional[str] = None
    calendly_emergency_event_type_uri: Optional[str] = None
    calendly_new_patient_event_type_uri: Optional[str] = None
    calendly_scheduling_url: str = "https://calendly.com/your-practice/new-patient-exam"

    # --- Google Workspace (OAuth 2.0 desktop-app credentials) -----------------
    #: HIPAA note: a BAA requires Google Workspace Business Plus or Enterprise —
    #: see startup_warnings() and the README's HIPAA checklist.
    google_credentials_path: str = "/app/credentials.json"
    google_token_path: str = "/app/token.json"
    google_primary_calendar_id: str = "primary"
    google_recall_tracking_calendar_id: Optional[str] = None
    google_treatment_plan_calendar_id: Optional[str] = None
    google_on_call_calendar_id: Optional[str] = None
    google_front_desk_calendar_id: Optional[str] = None
    google_drive_folder_id: Optional[str] = None
    #: Business Profile location resource name, e.g. "accounts/123/locations/456".
    google_business_profile_location: Optional[str] = None
    #: Address that the *Microns AI* service account drafts approval emails to
    #: (the dentist/front desk), never the practice's own inbox directly.
    dentist_approval_email: Optional[str] = None
    front_desk_email: Optional[str] = None
    insurance_coordinator_email: Optional[str] = None
    on_call_dentist_email: Optional[str] = None

    # --- Practice profile ------------------------------------------------------
    practice_name: str = "Microns Dental"
    practice_timezone: str = "America/New_York"
    practice_phone: Optional[str] = None
    practice_address: Optional[str] = None
    practice_booking_url: str = "https://your-practice.com/book"
    practice_google_review_url: str = "https://g.page/r/your-google-review-link/review"
    practice_open_hour: int = 8
    practice_close_hour_mon_thu: int = 17
    practice_close_hour_fri: int = 14
    appointment_slot_minutes: int = 30

    # --- Retention tuning --------------------------------------------------
    hygiene_recall_days: int = 180
    review_request_delay_days: int = 1
    review_recheck_days: int = 5

    # --- n8n ---------------------------------------------------------------
    #: Base URL for n8n webhook triggers. Set to the ``/webhook-test`` variant
    #: while building workflows in the editor. The 6 native-app n8n workflows
    #: shipped in ``n8n-workflows/`` are self-contained (they talk to Google
    #: Calendar/Gmail/Twilio/Calendly directly and do not call this backend),
    #: so this is only used if you choose to wire notifications from here.
    ...
    def calendly_enabled(self) -> bool:
        return bool(self.calendly_api_key)

    @property
    def google_enabled(self) -> bool:
        import os

        return os.path.exists(self.google_token_path) or os.path.exists(self.google_credentials_path)
    ...
    def startup_warnings(self) -> List[str]:
        ...
        if not self.openai_enabled:
            warnings.append(
                "OPENAI_API_KEY is not set — lead qualification, insurance-reply parsing "
                "and review replies fall back to deterministic/rule-based logic."
            )
        ...
        if not self.google_enabled:
            warnings.append(
                "No Google OAuth token found at GOOGLE_TOKEN_PATH — Calendar/Contacts/Gmail/"
                "Drive calls will fail until you run the OAuth flow (see README)."
            )
