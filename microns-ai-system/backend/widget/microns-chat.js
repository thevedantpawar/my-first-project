/**
 * Microns chat widget — embeddable lead qualification for med spa websites.
 *
 * Embed:
 *   <script
 ...
 * shadow root so the clinic's own CSS cannot break it and it cannot break
...
(function () {
  ...
  var config = {
    ...
    clinic: (script && script.getAttribute('data-clinic')) || 'our clinic',
    color: (script && script.getAttribute('data-color')) || '#0f766e',
    title: (script && script.getAttribute('data-title')) || 'Chat with us',
    position: (script && script.getAttribute('data-position')) || 'right'
  };

  var STORAGE_KEY = 'microns_chat_session';
  ...
  host.id = 'microns-chat-widget';
  host.setAttribute('data-microns', 'chat');
  ...
  wrap.innerHTML = [
    '<div class="panel" role="dialog" aria-modal="false" aria-label="Chat with ' + esc(config.clinic) + '">',
    '  <div class="head">',
    '    <div><h2>' + esc(config.clinic) + '</h2><p>Typically replies instantly</p></div>',
    ...
    '  </form>',
    '  <div class="legal">Please don\'t share medical details here — a provider will follow up.</div>',
    '</div>',
      ...
      .catch(function (error) {
        ...
        addMessage(
          error && error.message === 'rate_limited'
            ...
            : "Sorry, I couldn't reach the clinic just now. Please try again, or call us directly.",
          'error'
        );
        setBusy(false);
      });
  ...
  // Small public API so a clinic's own CTA can open the chat:
  //   window.MicronsChat.open()
  window.MicronsChat = {
    open: open,
    close: close,
    reset: function () {
